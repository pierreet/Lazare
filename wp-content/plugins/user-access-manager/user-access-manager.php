...
if (isset($oUserAccessManager)) {
	...
	$result = mysql_query('SHOW COLUMNS FROM  `'.$wpdb->prefix.'uam_accessgroups` LIKE  \'groupaddress\'');
	$exists = (mysql_num_rows($result))?TRUE:FALSE;
	if(!$exists)
		mysql_query('ALTER TABLE `'.$wpdb->prefix.'uam_accessgroups` ADD groupaddress VARCHAR(300) AFTER groupdesc');

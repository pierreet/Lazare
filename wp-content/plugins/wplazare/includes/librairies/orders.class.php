	...
	function pageTitle()
	{
		...
		if($action != '')
		{
			if(($action == 'edit') || ($action == 'view') || ($action == 'delete'))
			{
				...
				$title = sprintf(__('Voir la commande "%s"', 'wplazare'), $editedItem->order_reference);
			}
		}
		return $title;
	}
	...
	*/
	function elementAction()
	{
		...
		if(($action != '') && ($action == 'deleteok') && ($saveditem > 0))
		{
			...
			$pageMessage = '<img src="' . WPLAZARE_SUCCES_ICON . '" alt="action success" class="wplazarePageMessage_Icon" />' . sprintf(__('La commande "%s" a &eacute;t&eacute; supprim&eacute;e avec succ&eacute;s', 'wplazare'), '<span class="bold" >' . $editedElement->order_reference . '</span>');
		}
		...
		if($pageAction == 'validatePaiement')
		{
			if(current_user_can('wplazare_delete_orders'))
			{
				...
				$currentOrder = wplazare_orders::getElement($id, "'valid'", 'order_reference');
	...
	*/
	function paymentReturn()
	{
		...
		if($reference != '')
		{
			...
			$currentOrder = wplazare_orders::getElement($reference, "'valid'", 'order_reference');
	...
	function prepareBalisesReplace($currentOrder){
		...
		$balises_replace = array( 
			...
			array( "balise" => "{NUMERO_RECU}", "new_text" => date('Ym').substr_replace("00000",$currentOrder->order_reference, -strlen($currentOrder->order_reference)) ),
    ...
    {
        $currentOrderFull = wplazare_orders::getElement($reference, "'valid'", 'order_reference');
        $file_path = wplazare_orders::buildAutorisationPrelevement($currentOrderFull);

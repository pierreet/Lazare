...
Plugin Name: Incription
Description: DO NOT UNINSTALL !!
Author: Pierre-Etienne Crépy from Cform by Olivier Seidel
Version: 1.0
...
function settings_corrupted() {
	...
	if (function_exists('add_menu_page')){
		add_menu_page(__('Inscription', 'cforms'), __('Inscription', 'cforms'), 'manage_cforms', $tmp.'/cforms-corrupted.php', '', get_cf_plugindir().'/'.$tmp.'/images/cformsicon.png' );
	...
	elseif (function_exists('add_management_page'))
		add_management_page(__('Inscription', 'cforms'), __('Inscription', 'cforms'), 'manage_cforms', $tmp.'/cforms-corrupted.php');
...
function cforms($args = '',$no = '') {
	...
	### form title
	function form_name_after($no){
		global $cformsSettings;
		$name = '';
		if($cformsSettings['form'.$no]['cforms'.$no.'_mp']['mp_form']){
			$name = ($cformsSettings['form'.$no]['cforms'.$no.'_mp']['mp_next']!=-1)?' > '.$cformsSettings['form'.$no]['cforms'.$no.'_mp']['mp_next']:NULL;
			for($i=NULL; $i<=$cformsSettings['global']['cforms_formcount']; $i++){
				if($cformsSettings['form'.$i]['cforms'.$i.'_fname']==$cformsSettings['form'.$no]['cforms'.$no.'_mp']['mp_next']){
					$name .= form_name_after($i);
					break;
				}
			}
		}
	...
	function form_name_before($no){
		global $cformsSettings;
		$name = '';			
		for($i=NULL; $i<=$cformsSettings['global']['cforms_formcount']; $i++){
			if($cformsSettings['form'.$i]['cforms'.$i.'_mp']['mp_form'] && $cformsSettings['form'.$i]['cforms'.$i.'_mp']['mp_next']==$cformsSettings['form'.$no]['cforms'.$no.'_fname']){				
					$name = ' > '.$cformsSettings['form'.$i]['cforms'.$i.'_fname'];
					$name = form_name_before($i).$name;
					break;
			}
		}
		return $name;
	...
	$title .= '<span class="active"> >> '.$cformsSettings['form'.$no]['cforms'.$no.'_fname'].'</span>';
	$title = form_name_before($no).$title;
	$title .=form_name_after($no);	
    ...
    if( $cformsSettings['form'.$no]['cforms'.$no.'_mp']['mp_form'] && $cformsSettings['form'.$no]['cforms'.$no.'_mp']['mp_reset'] )
		$reset = '<input tabindex="999" type="submit" name="resetbutton'.$no.'" id="resetbutton'.$no.'" class="resetbutton" value="' . $cformsSettings['form'.$no]['cforms'.$no.'_mp']['mp_resettext'] . '" onclick="return confirm(\''.__('Note: Cela va tout effacer !', 'cforms').'\')">';
...
function cforms_style() {
  ...
  $exclude = ($cformsSettings['global']['cforms_inexclude']['ex']=='1');
	...
	if( $onPages=='' || (in_array($page_obj->ID,$onPagesA) && !$exclude) || (!in_array($page_obj->ID,$onPagesA) && $exclude)){
		...
		echo "\n<!-- Start Script Generated By Insc v".$localversion." -->\n";
echo "<script type='text/javascript' src='".site_url()."/wp-includes/js/jquery/ui/jquery.ui.datepicker.min.js'></script>";
				...
				"\t".'cfCAL.ROOT="'.$cformsSettings['global']['cforms_root'].'";' ."\n\n".
				 ...
				 "\t\t".'if( jQuery.datepicker ){'."\n".
					...
					"\t\t\t".'jQuery(".cf_date").datepicker({'."\n".
						"\t\t\t\t".'"buttonImage": cfCAL.ROOT+"/js/calendar.gif", buttonImageOnly: true, buttonText: cfCAL.TEXT_CHOOSE_DATE, showOn: "both", yearRange:"c-40:c+0",'."\n".
						"\t\t\t\t".'"dateFormat": cfCAL.dateFormat, "dayNamesMin": cfCAL.dayNames, "dayNamesShort": cfCAL.dayNames, "monthNames": cfCAL.monthNames, "firstDay":cfCAL.firstDayOfWeek,'."\n".
						"\t\t\t\t".'"nextText": cfCAL.TEXT_NEXT_MONTH, "prevText": cfCAL.TEXT_PREV_MONTH, "closeText": cfCAL.TEXT_CLOSE, changeYear: true, changeMonth: true});'."\n".

						
					"\t\t\t".'jQuery(".cf_agedate").datepicker({'."\n".
						"\t\t\t\t".'"buttonImage": cfCAL.ROOT+"/js/calendar.gif", buttonImageOnly: true, buttonText: cfCAL.TEXT_CHOOSE_DATE, showOn: "both", yearRange:"c-40:c+0",'."\n".
						"\t\t\t\t".'"dateFormat": cfCAL.dateFormat, "dayNamesMin": cfCAL.dayNames, "dayNamesShort": cfCAL.dayNames, "monthNames": cfCAL.monthNames, "firstDay":cfCAL.firstDayOfWeek,'."\n".
						"\t\t\t\t".'"nextText": cfCAL.TEXT_NEXT_MONTH, "prevText": cfCAL.TEXT_PREV_MONTH, "closeText": cfCAL.TEXT_CLOSE, changeYear: true, changeMonth: true});'."\n".
					'var cur_date = (!"'.$cformsSettings['form'.$no]['cforms'.$no.'_limitage'].'" && "'.$cformsSettings['form'.$no]['cforms'.$no.'_limitage'].'".length === 0 )?new Date():getDate("'.$cformsSettings['form'.$no]['cforms'.$no.'_limitage'].'");';

						
						
						if(!empty($cformsSettings['form'.$no]['cforms'.$no.'_limitagemax'])  && $cformsSettings['form'.$no]['cforms'.$no.'_limitagemax'] > 0){
							echo 'var cur_date = (!"'.$cformsSettings['form'.$no]['cforms'.$no.'_limitage'].'" && "'.$cformsSettings['form'.$no]['cforms'.$no.'_limitage'].'".length === 0 )?new Date():getDate("'.$cformsSettings['form'.$no]['cforms'.$no.'_limitage'].'");'.
							'var date_min = cur_date;date_min.setYear(date_min.getYear()-'.$cformsSettings['form'.$no]['cforms'.$no.'_limitagemax'].');'.	
							 'jQuery(".cf_agedate").datepicker("option", "minDate", date_min);';						
						}
						if(!empty($cformsSettings['form'.$no]['cforms'.$no.'_limitagemin']) && $cformsSettings['form'.$no]['cforms'.$no.'_limitagemin'] > 0){
							echo 'var cur_date = (!"'.$cformsSettings['form'.$no]['cforms'.$no.'_limitage'].'" && "'.$cformsSettings['form'.$no]['cforms'.$no.'_limitage'].'".length === 0 )?new Date():getDate("'.$cformsSettings['form'.$no]['cforms'.$no.'_limitage'].'");'.
							'var date_max = cur_date;date_max.setYear(date_max.getYear()-'.$cformsSettings['form'.$no]['cforms'.$no.'_limitagemin'].');'.	
							 'jQuery(".cf_agedate").datepicker("option", "maxDate", date_max);';
						}
						
						echo "\t\t\t".' if(jQuery.isFunction(jQuery(this).datetimepicker)) jQuery(".cf_datetime").datetimepicker({"firstDay":cfCAL.firstDayOfWeek, minDate: new Date(), "dateFormat": cfCAL.dateFormat,"dayNamesMin": cfCAL.dayNames, "dayNamesShort": cfCAL.dayNames, "monthNames": cfCAL.monthNames,
						onSelect: function(a,b){
						                if(jQuery.isFunction(jQuery(this).datetimepicker)){
										var date = new Date(jQuery(this).datetimepicker(\'getDate\'));
										var day = date.getDay(), Sunday = 0, Monday = 1, Tuesday = 2, Wednesday = 3, Thursday = 4, Friday = 5, Saturday = 6;
										
										var restrictionDays = [';
											echo $cformsSettings["global"]["cforms_rdv_lun"]?'[[Monday],[\''.$cformsSettings["global"]["cforms_rdv_lun_from"].'\'],[\''.$cformsSettings["global"]["cforms_rdv_lun_to"].'\']],':NULL;
											echo $cformsSettings["global"]["cforms_rdv_mar"]?'[[Tuesday],[\''.$cformsSettings["global"]["cforms_rdv_mar_from"].'\'],[\''.$cformsSettings["global"]["cforms_rdv_mar_to"].'\']],':NULL;
											echo $cformsSettings["global"]["cforms_rdv_mer"]?'[[Wednesday],[\''.$cformsSettings["global"]["cforms_rdv_mer_from"].'\'],[\''.$cformsSettings["global"]["cforms_rdv_mer_to"].'\']],':NULL;
											echo $cformsSettings["global"]["cforms_rdv_jeu"]?'[[Thursday],[\''.$cformsSettings["global"]["cforms_rdv_jeu_from"].'\'],[\''.$cformsSettings["global"]["cforms_rdv_jeu_to"].'\']],':NULL;
											echo $cformsSettings["global"]["cforms_rdv_ven"]?'[[Friday],[\''.$cformsSettings["global"]["cforms_rdv_ven_from"].'\'],[\''.$cformsSettings["global"]["cforms_rdv_ven_to"].'\']],':NULL;
											echo $cformsSettings["global"]["cforms_rdv_sam"]?'[[Saturday],[\''.$cformsSettings["global"]["cforms_rdv_sam_from"].'\'],[\''.$cformsSettings["global"]["cforms_rdv_sam_to"].'\']],':NULL;
											echo $cformsSettings["global"]["cforms_rdv_dim"]?'[[Sunday],[\''.$cformsSettings["global"]["cforms_rdv_dim_from"].'\'],[\''.$cformsSettings["global"]["cforms_rdv_dim_to"].'\']],':NULL;
											echo '];';
									echo	'
										var i = 0
										while(day != restrictionDays[i][0])
											i++;
											
										var hMin = (restrictionDays[i][1].toString().substring(0,2)===\'\')?0:restrictionDays[i][1].toString().substring(0,2);
										var mMin = restrictionDays[i][1].toString().substring(3,5);
										var hMax = (restrictionDays[i][2].toString().substring(0,2)===\'\')?23:restrictionDays[i][2].toString().substring(0,2);
										var mMax = restrictionDays[i][2].toString().substring(3,5);
										
										while(date.getHours()<hMin)
											date.setHours(date.getHours()+jQuery(this).datepicker("option", "stepHour"));
										while(date.getHours()>hMax)
											date.setHours(date.getHours()-jQuery(this).datepicker("option", "stepHour"));
										while(date.getMinutes()<mMin && date.getHours()==hMin)
						...
						beforeShowDay: function(date){
										  var day = date.getDay(), Sunday = 0, Monday = 1, Tuesday = 2, Wednesday = 3, Thursday = 4, Friday = 5, Saturday = 6;
											...
											var closedDays = [';
											echo !$cformsSettings["global"]["cforms_rdv_lun"]?'[Monday],':NULL;
											echo !$cformsSettings["global"]["cforms_rdv_mar"]?'[Tuesday],':NULL;
											echo !$cformsSettings["global"]["cforms_rdv_mer"]?'[Wednesday],':NULL;
											echo !$cformsSettings["global"]["cforms_rdv_jeu"]?'[Thursday],':NULL;
											echo !$cformsSettings["global"]["cforms_rdv_ven"]?'[Friday],':NULL;
											echo !$cformsSettings["global"]["cforms_rdv_sam"]?'[Saturday],':NULL;
											echo !$cformsSettings["global"]["cforms_rdv_dim"]?'[Sunday],':NULL;
											echo '];';
											
									echo	'for (var i = 0; i < closedDays.length; i++) {
						...
						"\t\t\t".'if(jQuery.isFunction(jQuery(this).timepicker)) jQuery(".cf_time").timepicker({});'."\n".
...
add_filter('wp_footer', 'cforms_scripts',99);
?>
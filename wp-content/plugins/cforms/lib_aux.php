...
### write DB record
function write_tracking_record($no,$field_email,$field_fname,$field_lname,$c=''){
		...
		if ( $cformsSettings['global']['cforms_database'] == '1' ) {
			...
			if($cformsSettings['form'.$no]['cforms'.$no.'_detail']){
					$subID = str_replace('xP312qNW', '', substr($_REQUEST['sub_id'], 8));
...
function check_cust_vars($m,$t,$no, $html=false) {
	...
	if ( count($findall[1]) > 0 ) {
		...
		$allvars = array();
		foreach($allvar as $v){
			$allvars[] = preg_replace('/cf_form[0-9]*_/', '', $v);
		}
		$ot = $t;
		$t = array();
		foreach($ot as $k => $u){
			$t[preg_replace('/cf_form[0-9]*_/', '', $k)] = $u;
		}
		foreach ( $findall[1] as $fvar ) {
			...
			### ged
			if ( strpos($fvar,'_documents')!==false ){

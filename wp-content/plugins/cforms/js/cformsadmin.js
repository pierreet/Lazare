...
function submissions_loaded(){
	...
	jQuery('.accept','#entries').bind("click", accept_submission );
	jQuery('.deny','#entries').bind("click", deny_submission );
	jQuery('.desist','#entries').bind("click", desist_submission );
	jQuery('.rdv','#entries').bind("click", rdv_submission );
	jQuery('.rdv_accept','#entries').bind("click", rdv_accept_submission );
	jQuery('.rdv_deny','#entries').bind("click", deny_submission );
	jQuery('.ged_submit','#entries').bind("click", ged_submission );
...
*/
	...
	jQuery(".checkable").editInPlace({
		bg_out:'#dddddd',
		bg_over:'#f7f7f7',
		use_html: true,
		field_type: 'input type="checkbox"',		
		url: jQuery('#geturl').attr('title')+'lib_database_savedata.php',
		saving_image: jQuery('#geturl').attr('title')+'../css/images/load.gif',
		textarea_cols:'30',
		textarea_rows:'4',
...
function deny_submission(){
	...
	var that = this;
	jQuery.post(jQuery('#geturl').attr('title')+'lib_database_adddata.php', {sub_id: eid, field_name: 'Réponse', field_val: 'Refusé(e)'}, function(data){ document.location.href = window.location.pathname+jQuery(that).attr('href'); } );
	return false;
}
function desist_submission(){
	var i = 1;
	eid = this.id.substr(i,this.id.length);
	while(isNaN(eid))
		eid = this.id.substr(i++,this.id.length);
	...
	var that = this;
	jQuery.post(jQuery('#geturl').attr('title')+'lib_database_adddata.php', {sub_id: eid, field_name: 'Désistement', field_val: 'Oui'}, function(data){ document.location.href = window.location.pathname+jQuery(that).attr('href'); } );
	return false;
}


function rdv_submission(){
	var that = this;
	var i = 1;
	eid = this.id.substr(i,this.id.length);
	while(isNaN(eid))
		eid = this.id.substr(i++,this.id.length);
	var dates = jQuery(this).attr('dates').split(',');
	var options ='';
	...
	jQuery('#dialog').html('Veuillez choisir une date pour le rendez-vous :<br />'+
							'<input type="radio" name="type" value="liste" checked="checked">'+
							'Dans la liste <select id="date" name="date">'+options+'</select><br />'+
							'<input type="radio" name="type" value="autre">'+
							'Une autre <input type="text" id="autre" name="autre" class="cf_datetime" id="cforms_datetime">'
							);
	 dateTimeInput("cf_datetime"); // lib_function.php
							
	jQuery('#dialog').dialog({ autoOpen: true, title: "Rendez-vous", buttons: { "Ok": function() { 
																								if(jQuery('input[type=radio][name=type]:checked').attr('value')=='liste'){
																										var date = jQuery('#date').val();
																									}else{
																										var date = jQuery('#autre').val();
																									}
																						jQuery(this).dialog("close");
																						jQuery.post(jQuery('#geturl').attr('title')+'lib_database_adddata.php', 
																							{sub_id: eid, field_name: 'Rendez-vous fixé le', 
																							field_val: date}, function(data){  
																												document.location.href = window.location.pathname+encodeURI(decodeURI(jQuery(that).attr('href')).replace("{_date}", dateFormat(getDate(date),'dddd d mmmm yyyy à HH:MM')));
																											} );
																					}																					
...
function ged_submission(){

	var ged = '';
	jQuery('.ged').each(function(){
		if(jQuery(this).attr('state')=='false')
			ged = ged + jQuery(this).attr('name')+';<br /> ';
	});
	ged = ged.replace(/^\s+/g,'').replace(/,\s+$/g,'');	
	document.location.href = window.location.pathname+encodeURI(decodeURI(jQuery(this).attr('href')).replace("{_documents_manquants}", ged));
	return false;

}

function rdv_accept_submission(){
	var i = 1;
	eid = this.id.substr(i,this.id.length);
	while(isNaN(eid))
		eid = this.id.substr(i++,this.id.length);
	...
	var that = this;
	 jQuery.post(jQuery('#geturl').attr('title')+'lib_database_adddata.php', {sub_id: eid, add_document: 'true'}, function(data){  document.location.href = window.location.pathname+jQuery(that).attr('href');} );
	 return false;
}
	...
	$.addFlex = function (t, p) {
		...
		if (p.title) {
			...
			if (p.showTableToggleBtn) {
				$(g.mDiv).append('<div class="ptogtitle" title="Rꥵire/Agrandire"><span></span></div>');
...
Authors:
	Dave Hauenstein
	Martin H夫er <spamfaenger [at] gmx [dot] de>
...
$.fn.editInPlace.defaults = {
	...
	save_button:		'<button class="inplace_save">Save</button>', // string: image button tag to use as ԓaveԠbutton
	cancel_button:		'<button class="inplace_cancel">Cancel</button>', // string: image button tag to use as ԃancelԠbutton
	...
	default_text:		"(Cliquer pour ajouter du texte)", // string: text to show up if the element that has this functionality is empty

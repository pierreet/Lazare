...
if ($sub_ids<>'') {
	...
	header("Content-Disposition: attachment; filename=\"inscriptions." . $format . "\"");
	header("Content-Transfer-Encoding: binary");
...
function getCSVTAB($format='csv'){
	...
	*/
	...
	### while( $entry = mysql_fetch_array($r) ){
		...
		$entry[field_val] = ($entry[field_val]=='false')?'Non':$entry[field_val];
		$entry[field_val] = ($entry[field_val]=='true')?'Oui':$entry[field_val];
		...
		if( $sub_id<>$entry[id] ){   ### new record starts
			...
			$body  = encData($entry[sub_date]) .'"' . $format . ($_GET['addip']=='true'?$entry[ip].$format:'');
			$head  = ($_GET['header']=='true')? "Date d'inscription" . $format . $ipTab:'';
			$last_n = $next_n;

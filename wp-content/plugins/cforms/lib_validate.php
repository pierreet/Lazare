...
for($i = 1; $i <= $field_count; $i++) {
		...
		if( $field_emailcheck ) {  ### email field
		...
		else if( $field_telcheck ) {  ### tel field

					$validations[$i+$off] = cforms_is_tel( $current_field ) || (!$field_required && $current_field=='');
				...
				if( in_array($field_type,array('cauthor','url','comment','pwfield','textfield','datepicker','textarea','yourname','youremail','friendsname','friendsemail','agedatepicker','datetimepicker')) ){
...
function cforms_is_tel($string){
	return eregi("^0[1-9]([-. ]?[0-9]{2}){4}$", $string);
}

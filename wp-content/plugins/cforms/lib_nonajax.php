...
if( isset($_POST['sendbutton'.$no]) && $all_valid ) {
		...
		for($i = 1; $i <= $field_count; $i++) {
			...
			if ( $field_fname == '' && preg_match("/^prénom/i",trim($field_name)))
				$field_fname = $_POST[$current_field];
				
			if ( $field_lname == '' && preg_match("/^nom/i",trim($field_name)))
				$field_lname = $_POST[$current_field];		
	...
	if( $cformsSettings['form'.$no]['cforms'.$no.'_mp']['mp_form'] ){
		...
		if( $field_fname<>'' )
       		$_SESSION['cforms']['fname']=$field_fname;
		if( $field_lname<>'' )
       		$_SESSION['cforms']['lname']=$field_lname;
		...
		$field_fname = $_SESSION['cforms']['fname']; ### fetch from prev. def
		$field_lname = $_SESSION['cforms']['lname']; ### fetch from prev. def
		$inSession = '1';
	}
	...
	###
	$subID = ( $isTAF =='2' && !$send2author )?'noid':write_tracking_record($no,$field_email,$field_fname,$field_lname);

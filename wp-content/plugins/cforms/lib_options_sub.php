<?php
	...
	for($i = 1; $i <= $field_count; $i++) {
        ...
        if( !$uploadfield )
          $uploadfield = ($type == 'upload');
				...
				if(isset($_REQUEST['field_' . $i . '_required']) && in_array($type,array('pwfield','textfield','datepicker','textarea','checkbox','multiselectbox','selectbox','emailtobox','upload','yourname','youremail','friendsname','friendsemail','email','cauthor','url','comment','radiobuttons','agedatepicker', 'datetimepicker')) ) {
					$required = 1;
				}
				...
				if(isset($_REQUEST['field_' . $i . '_telcheck']) && in_array($type,array('textfield','datepicker','youremail','friendsemail','email')) && $emailcheck != 1){
					$telcheck = 1;
				}
				...
				$all_fields[$i-1] = $name . '$#$' . $type . '$#$' . $required. '$#$' . $emailcheck . '$#$'. $clear . '$#$' . $disabled . '$#$' . $readonly . '$#$' . $telcheck;
  ...
  $cformsSettings['form'.$no]['cforms'.$no.'_noattachments'] =  $_REQUEST['cforms_noattachments']?'1':'0';
	...
	$cformsSettings['form'.$no]['cforms'.$no.'_limitage'] =	 				$_REQUEST['cforms_limitage'];
	
	$cformsSettings['form'.$no]['cforms'.$no.'_maxentries'] =	 				$_REQUEST['cforms_maxentries'];
	...
	if( isset($_REQUEST['AddDoc']) ){
			...
			$cformsSettings['global']['ged'] .= 'Nouveau document;';

	}
	
    update_option('cforms_settings',$cformsSettings);
	echo '<div id="message" class="updated fade"><p>'.__('Paramètres mis à jour.', 'cforms').'</p>'.$usermsg.'</div>';
?>
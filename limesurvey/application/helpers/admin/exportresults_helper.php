    ...
    function exportSurvey($iSurveyId, $sLanguageCode, $sExportPlugin, FormattingOptions $oOptions, $sFilter)
    {
        ...
        switch ( $sExportPlugin ) {
            ...
            case "pdf":
                ...
                {
                    $writer = new PdfWriter($oOptions->fileName);
                }
                else
                {
...
class PdfWriter extends Writer
{
    ...
    public function outputRecord($headers, $values, FormattingOptions $oOptions)
    {
        ...
        elseif ($oOptions->answerFormat == 'long')
        {
            ...
            foreach($headers as $header)
            {
            	$this->pdf->SetFont('times', 'B', Yii::app()->getConfig('pdffontsize'));
                $this->pdf->intopdf($header);
                $this->pdf->SetFont(Yii::app()->getConfig('pdfdefaultfont'), '', Yii::app()->getConfig('pdffontsize'));
                if($header == 'Photo'){
                	$json = substr($values[$columnCounter],1,strlen($values[$columnCounter])-2);
                	$tmpdir = 'C:/php/Workspace/limesurvey/upload/surveys/758661/files/';//Yii::app()->getConfig('uploaddir') . DIRECTORY_SEPARATOR."surveys". DIRECTORY_SEPARATOR . $iSurveyID . DIRECTORY_SEPARATOR."files".DIRECTORY_SEPARATOR;
                	$obj = json_decode($json);
                	if($obj->{'ext'} == 'png') {
                		$this->pdf->AddPage();
                		$this->pdf->Image($tmpdir.$obj->{'filename'}, '', '', '', '', 'PNG', '', 'T', true, 150, '', false, false, 1, false, false, false);
                		$this->pdf->AddPage();
                	}
                	else $this->pdf->intopdf($obj->{'name'});
                }
                else{
	                if($this->stripTagsFull($values[$columnCounter]) == '')
	                	$this->pdf->intopdf('Pas de réponse');
	                else $this->pdf->intopdf($this->stripTagsFull($values[$columnCounter]));
                }
                $this->pdf->Ln(5);
                $columnCounter++;
            }
